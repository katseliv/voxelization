...
public class VoxelOperation {
    ...
    public boolean isInnerPoint(Vector3 point) {
        int numberX = getNumberOfIntersectionsWithSurface(point, Condition.GET_Y, Condition.GET_Z) - 1;
        int numberY = getNumberOfIntersectionsWithSurface(point, Condition.GET_X, Condition.GET_Z) - 1;
        int numberZ = getNumberOfIntersectionsWithSurface(point, Condition.GET_X, Condition.GET_Y) - 1;
    ...
    private int getNumberOfIntersectionsWithSurface(Vector3 vector, Condition c1, Condition c2) {
        int counter = 0;

        for (MyPolygon pl : polygons) {
            LinkedList<Vector2> points = new LinkedList<>();

            points.add(new Vector2(getCoordinate(pl.getPoint1(), c1), getCoordinate(pl.getPoint1(), c2)));
            points.add(new Vector2(getCoordinate(pl.getPoint2(), c1), getCoordinate(pl.getPoint2(), c2)));
            points.add(new Vector2(getCoordinate(pl.getPoint3(), c1), getCoordinate(pl.getPoint3(), c2)));

            if (calculateSquare(points.get(0), points.get(1), points.get(2)) != 0) {
                counter += triangleContainThisPoint(points, new Vector2(getCoordinate(vector, c1), getCoordinate(vector, c2)));
            }
        }
    ...
    private float getCoordinate(Vector3 vector, Condition c) {
        switch (c) {
            case GET_X:
                return vector.getX();
            case GET_Y:
    ...
    public List<MyPolygon> surfaceToVoxelOfPolygon(Surface surface, float height, float sizeVoxel) {
        polygons = surface.getPolygons();
        List<MyPolygon> polygonsOfModel = new LinkedList<>();
        ...
        for (float i = surface.getStartPoint().getX() + sizeVoxel / 2; i < xMax; i += sizeVoxel) {
            for (float j = surface.getStartPoint().getY() + sizeVoxel / 2; j < yMax; j += sizeVoxel) {
                ...
                for (float k = max - sizeVoxel; k < max; k += sizeVoxel) {
                    polygonsOfModel.addAll(new Parallelepiped(sizeVoxel / 2, new Vector3(i, j, k)).getPolygons());
                }
